...
#add for 1.1.x code of rocci server, to avoid error "bundle install after changing gemfile"
...
#new added based on 1.1.x of rocci-server code
...
sed -i 's/SetEnv ROCCI_SERVER_EC2_AWS_AVAILABILITY_ZONE/#SetEnv ROCCI_SERVER_EC2_AWS_AVAILABILITY_ZONE/g' /etc/apache2/sites-available/occi-ssl.conf 
sed -i '/#SetEnv ROCCI_SERVER_EC2_AWS_AVAILABILITY_ZONE/a\    SetEnv ROCCI_SERVER_EC2_AWS_AVAILABILITY_ZONE      us-east-1b' /etc/apache2/sites-available/occi-ssl.conf
